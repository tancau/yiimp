...
static int decred_parse_header(YAAMP_JOB_TEMPLATE * templ,
  const char * header_hex, bool getwork) {
...
bool coind_create_job(YAAMP_COIND * coind, bool force) {
  ...
  uint64_t coin_target = decode_compact(templ - > nbits);
  if (templ - > nbits && !coin_target) coin_target = 0xFFFF000000000000 ULL; // under decode_compact min diff
