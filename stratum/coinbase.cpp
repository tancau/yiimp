...
static void script_pack_tx(YAAMP_COIND * coind, char * data, json_int_t amount,
  const char * script) {
  char evalue[32];
  char coinb2_part[256];
  char coinb2_len[4];
...
void coinbase_create(YAAMP_COIND * coind, YAAMP_JOB_TEMPLATE * templ, json_value * json_result) {
  ...
  if (strcmp(coind - > symbol, "EGC") == 0) {
    ...
    char script_dests[2048] = {
      0
    };
    char script_payee[128] = {
      0
    };
    char payees[3];
    ...
    case 0:
      sprintf(coind - > charity_address, "3K3bPrW5h7DYEMp2RcXawTCXajcm4ZU9Zh");
      break;
    case 1:
      sprintf(coind - > charity_address, "33Ssxmn3ehVMgyxgegXhpLGSBpubPjLZQ6");
      break;
    case 2:
      sprintf(coind - > charity_address, "3HFPNAjesiBY5sSVUmuBFnMEGut69R49ca");
      break;
    case 3:
      sprintf(coind - > charity_address, "37jLjjfUXQU4bdqVzvpUXyzAqPQSmxyByi");
      break;
    }
    ++npayees;
    ...
    if (!coind - > hasmasternodes && founder_enabled && founder) {
      char founder_payee[256] = {
        0
      };
      char founder_script[1024] = {
        0
      };
  ...
  if (strcmp(coind - > symbol, "XDN") == 0) {
    ...
    char masternode_script[1024] = {
      0
    };
    char founder_payee[256] = {
      0
    };
    char masternode_payee[256] = {
      0
    };
  ...
  if (strcmp(coind - > symbol, "FLO") == 0) {
    if (coind - > txmessage) {
      strcat(templ - > coinb2, "00");
    }
  } //fixes FlorinCoin
	if (strcmp(coind - > symbol, "GIO") == 0) {
		if (coind - > txmessage) {
			strcat(templ - > coinb2, "00");
		}
	} //fixes GravioCoin
