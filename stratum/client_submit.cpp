...
static void create_decred_header(YAAMP_JOB_TEMPLATE * templ, YAAMP_JOB_VALUES * out,
  const char * ntime,
    const char * nonce,
      const char * nonce2,
        const char * vote, bool usegetwork) {
...
static void client_do_submit(YAAMP_CLIENT * client, YAAMP_JOB * job, YAAMP_JOB_VALUES * submitvalues,
  ...
  if (templ - > nbits && !coin_target) coin_target = 0xFFFF000000000000 ULL;
...
void client_submit_error(YAAMP_CLIENT * client, YAAMP_JOB * job, int id,
  const char * message, char * extranonce2, char * ntime, char * nonce) {
...
bool client_submit(YAAMP_CLIENT * client, json_value * json_params) {
  ...
  if (is_decred && client - > extranonce2size > 4) {
    ...
    if (extranull && client - > extranonce2size > 8)
      extranull = (int) !strcmp( & extranonce2[8], "00000000"
        "00000000");
    if (extranull) {
